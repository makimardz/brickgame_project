...
class Director:
    ...
    def start_game(self):
        ...
        while True: # game loop
            if random.randint(0, 1) == 0:
                pygame.mixer.music.load(BRICKS_0_MUSIC)
            else:
                pygame.mixer.music.load(BRICKS_1_MUSIC)
    ...
    def _execute_actions(self):
        ...
        while True: # main game loop
            ...
            DISPLAY_SURF.fill(BACK_GROUND_COLOR)
            self.draw_brick_game_board(board)
            self.draw_game_board_stats(score, level)
            self.draw_next_brick_piece(next_piece)
            
            if falling_piece != None:
                self.draw_brick_piece(falling_piece)
    ...
    def draw_brick_game_board(self, board):
        """
    ...
    def draw_next_brick_piece(self, piece):
        """
        draw the "next" text
        """
        next_brick_color = BASIC_FONT.render('Next Shape:', True, TEXT_COLOR)
        next_tetris_shape = next_brick_color.get_rect()
        next_tetris_shape.topleft = (WINDOW_WIDTH - 120, 80)
        DISPLAY_SURF.blit(next_brick_color, next_tetris_shape)
        # draw the "next" piece
        self.draw_brick_piece(piece, pixelx=WINDOW_WIDTH - 120, pixely=100)
    

    def draw_brick_piece(self, piece, pixelx=None, pixely=None):
        brick_shape_to_draw = SHAPES[piece['shape']][piece['rotation']]
        ...
        for x in range(TEMPLATE_WIDTH):
            for y in range(TEMPLATE_HEIGHT):
                if brick_shape_to_draw[y][x] != BLANK:
